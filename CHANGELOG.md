...
- **修「完成能力测评后，个人中心不显示能力维度」**（本版主修复）：根因是「我的」/「首页」Tab 只在**首次进入**时拉一次画像与统计，而底部 Tab 切换用返回栈 `saveState/restoreState` 恢复、ViewModel 不重建——从「我的」点「去测评」→ 完成 → 回「我的」，看到的仍是测评前快照（雷达空白）。本版起两个 Tab 每次**恢复可见**（测评/练习返回、切回本 Tab、App 回前台）都会重新拉取：测评完立即看到四维雷达 + CEFR 徽章。
- **首页「未测评引导」卡即时消失**：测评完成后回到首页，「5 分钟测出你的英语水平」引导卡不再残留（同因：画像在返回时重拉）。
- **词汇 Tab 弱词数不再滞后**：做完「弱词训练」毕业一个词后回到词汇 Tab，弱词徽章即时更新（同 bug 类：本地 Room 数只在首次进入时读）。
- **空态文案更诚实**：「我的」画像卡按是否已测评区分文案——已测评的学员不再看到「先完成一次跟读或情景课」「完成一次测评即可补全画像」这类与实况矛盾的提示（判级/画像没写入时改说「继续练习即可点亮」）。

### 工程摘要

- **`MeScreen` / `HomeScreen` / `VocabHubScreen` 加 `LifecycleResumeEffect(Unit)`**：恢复可见即调 `viewModel.refresh()`（lifecycle-runtime-compose 2.8.7 稳定 API；NavHost 只组合栈顶目的地，返回/Tab 切回必触发）。幂等 GET/Room 读，刷新期间旧数据保持不闪烁；一次改动覆盖「测评返回 / 练习后返回 / 切 Tab / App 回前台」全部陈旧场景。
- **`MeViewModel` 降级条件收紧**：去掉 `|| current.assessed` 子句（git blame 溯源为 P7 首版引入）。当前后端下「已测评但四维全空」不可达（真判级必写 ≥3 维证据），删除后行为等价；口径更自洽——画像有真证据才不被 `GET /stats` 兜底覆盖，无证据一律允许兜底画「不完整」雷达。
- **`AssessmentResultHolder` KDoc 对齐实现**：删掉「holder 为空时自读 `GET /ability` 兜底」的过期描述——实际行为是错误态并提示去「我的-能力画像」查看；行为未改。
- **刻意不改（核查后记录）**：① `deviceId` 随系统备份/恢复＝「换机不丢进度」，是既有身份续档行为，未加提取规则排除（双设备同身份为低频低害场景，待账号体系引入再解）；② `AbilityProfile.userFound` 为 API 契约标记且被 DTO/单测钉住，保留。
- **测试基线**：Android JVM 单测 **326 passed** 全绿（本版为刷新时机与文案调整，纯逻辑未变，无新增用例）；ktlint 清零（CI 同版 1.3.1 CLI 口径）。
- ⏳ **真机验收**：我的 → 去测评 → 完成测评 → 回「我的」：雷达四维 + CEFR 徽章出现；首页引导卡消失；词汇 Tab 做完弱词训练返回徽章减少；断网切回 Tab 旧数据保留不白屏。
...
- **发布前冒烟揪出的二阶根因（同版修复）**：异步化部署后对生产 attempt 的判级冒烟 **24s 即落 stub**，日志 `LLM 调用失败: Request timed out.`——上一轮只放宽了**墙钟**（`hard_budget_s`→120s），而 `_judge` 传给 openai SDK 的**单次 socket 超时**仍是 `LLM_TIMEOUT_S=20s`（`max_retries=0`），先到先斩，作业预算形同虚设。教训入档：**墙钟与 socket 两级超时必须同扩**。新增 `ASSESSMENT_JUDGE_JOB_TIMEOUT_S=100.0`（介于同步 20s 与墙钟 120s 之间），`judge_level` 加 `timeout_s` 参数透传，作业调用点点名；**同步路径缺省不动（老客户端 30s 契约零变化）**。补 2 把静态锁 + provider 桩行为测试各钉住"缺省=20 / 覆写=100 真透传到 chat"。
...
- **录音条从"滚过去的波形"改成"当下的声量"**：v2.2.0 的滚动波形（40 根细条、≈1.6s 历史、松手定格形状）换成**实时声量脉冲条**（`RecordingPulseMeter`）——5 根中心高两端低的条跟着当前音量跳动（数据源是 `AudioRecorder.levelFlow` 的 VU 包络，快起慢落）；松手后整组**定格在本条峰值**并变淡，直到评分回来；空闲时画一条淡底轨。
- **评分反馈卡统一重做为中性卡**：打基础 `DrillFeedbackCard` / 跟读与角色对话 `ScorePanel` / 自由对话 `FreeScoreCard` 收敛到同一形状——**中性卡 + `ScoreRing` 分数环（分数带染色）+ 五维/三维子分进度条（`SubScorePill` 原地重样式）+ 逐词染色芯片（`WordChip`：色底，词 + 音标 + 分数）+ 建议/引擎转写引用块**；打基础评分卡新增入场动画。弱词训练卡经共用件自动受益。逐轮气泡条、影子跟读行内分、复盘页、成绩页、CEFR 一律不动。

### 工程摘要

- **滚动波形管线整体退役**：删除 `RecordingWaveform.kt` / `WaveformGeometry.kt` / `WaveformHistory.kt` 及两个对应测试文件——沿用 v2.2.0 删 `RecordingLevelIndicator` 的「编译器强制迁移」先例。「收工保留波形历史」的决策随缓冲区一起退役：定格峰值改由 UI 端捕获（`nextPeak` 纯函数 + JVM 单测钉住「`finalizeTake` 先清零、`isBusy` 后翻转」的竞态）。`AudioLevelMapping` 及其 **8** 个单测未动（此前文档写 9 个，是错的）。
- **无障碍文案诚实规则随组件迁移**：LIVE 态不提滚动/波形，FROZEN 态不说「正在录音」。
- **debug 包默认后端切到生产公网端点**：`BACKEND_BASE_URL`（debug）从模拟器回环 `http://10.0.2.2:8000/api/v1/` 改为 `http://118.89.58.84:5173/api/v1/`，与 release 同源——debug APK 装在任何手机上都开箱即用，不再指向"只有开发机模拟器才连得到"的地址（这是"模拟主机"反馈的根治）。本地联调走 App「设置」页覆盖 Base URL（设置页的默认值提示文案同步更新）。设置页覆盖逻辑、cleartext 许可、release 端点均未动。
- **强更补设（服务端配置，非代码）**：`backend/.env` 设 `APP_MIN_SUPPORTED_VERSION=2.2.0` 并 `deploy.sh restart`——v2.2.0 发布时按 SOP 应设而实测从未生效（OTA 一直返回哨兵 `0.0.0`/`force=false`），旧 v2.1.0 包在 finish-mission 202 契约下收工拿空 `review` 的暴露面就此关闭。**回滚 APK 时必须同步把它改回 `0.0.0` 哨兵**，否则用户被钉死在一个已下架的版本要求上（operations.md §3.2 的既有警示）。
- **测试基线**：Android JVM 单测 **318 passed**（预期 328 − 23（波形两测） + 13 = 318，实测全绿）；⚠️ 诚实口径：**开工时实测基线是 328**，旧文档写的 327 在开工前就已过期。后端 **573** 未动（零改动）。ktlint 干净。
- **已知边界**：`AssessmentScreens` 的 `isBusy=false` 硬编码保持原样——该页与 v2.2.0 行为一致（永不显示定格态），记为已知边界而非疏漏。
- ⏳ **v2.2.1 真机验收尚未执行**（与 v2.2.0 同款诚实口径）：脉冲条手感与评分卡观感目前只有源码 + JVM 单测作证据；发版状态以 `docs/operations.md`「发版状态与验收状态」节为准。
