...
|---|---|
| 日期 | 2026-07-11 |
| 状态 | **历史设计稿**（v1.4 已按此落地；v2.x 实现差异见**文末附录「v2.1.0 实现增补」**，正文保持原样不改）|
...
## 附录 A · v2.1.0 实现增补（2026-09-06，正文历史原貌不动）

本设计稿落地为 v1.x 后，v2.0/v2.1 按「对标可栗口语」大版本扩展。与初版设计的差异与现状，以本附录为准：

| 初版设计 | 落地状态（v2.1.0） |
|---|---|
| MVP=K12+新概念一册跟读，其余 8 场景预留 | 大幅扩展：跟读/角色对话/影子跟读/自由对话/弱词训练 五模式全部落地并产品化（Android UI 在 v1.4 即完成，比本稿计划快）；新概念语料最终仅 nce1/nce2 各 1 课示例 + 商务书 6 课（业务场景是真实需求锚点，K12 路线未继续） |
| 内容动态性=“场景对话由 LLM 动态生成”（§1.2，当时只落到自由对话） | **任务通关情景课体系**（v2.0 主打）：8 门人工剧本课 + CEFR 7 题测评 + 两段式 AI 生成课（说出目标→打基础四题型→任务清单实战→复盘报告），见 plans/1788164431817-eager-cactus.md |
| §3.1“客户端不直连任何外部 AI” | 完全遵守（所有 MiMo/ISE/IAT/百炼 调用均在后端，客户端零密钥）——与初版一致 |
| §5.1 实战对话的 “LLM 语义相关度评分” | 已实现且超出：单轮一次 LLM 综合调用返回 人设回复+示范句+任务累积判定+语法/词汇分+润色对照；发音维度=有参考 ISE 或转写锚定 ISE，无凭据整维不出证据（EWMA 画像 stub 门控，宁缺勿滥） |
| §5.3 L1 先做 6 端点 / L2 其余 | 全部落地且大幅扩展（34+ 端点）；端点全表见 backend/README |
| §8 “Redis 缓存” | v2.0 收尾整体移除——TTS 从未接 Redis，已删依赖；改磁盘缓存（static/tts 按 (text,voice)）+ 表达库/课程缓存走 Room 本地快照 |
| §10 分期 | 已越过本计划完成 8 轮交付（v1.2→v2.1.0），v2.1.0 关键增补：生产 :5173 + 自托管 OTA + 四 Tab 信息架构（首页/课程/词汇/我的）+ 暖栗主题 |
| §12 待确认 1/2 | 讯飞凭据仍未申请（当前环境用真实 ISE，stub 带「占位假分」警示，画像不被污染）；「配 key 即真」路径已固化在代码与文档（backend/README） |

当前运维事实：生产主实例 :5173（本盒防火墙唯一映射口）、生产库 english_prod_5173、OTA 自托管 `static/apk` + publish_apk.sh 发版必跑——全见 **docs/operations.md**。九阶段交付纪要：`.mimocode/tasks/NIGHTLY.md` 与 T1-T9 报告（过程档，gitignored）。
