...
| `ASSESSMENT_JUDGE_JOB_TIMEOUT_S` | 100.0 s | 同上作业的**单次 socket 上限**（`judge_level(timeout_s=)` → `_judge` → `provider.chat(timeout=)`）| **两级超时成对放宽**：墙钟 120 不配 socket 放宽 = 形同虚设。二阶根因实锤（2026-09-14 部署后第一发冒烟 24s 落 stub，日志 `LLM 调用失败: Request timed out.`）：openai SDK 按基座 `LLM_TIMEOUT_S=20` 先斩，限速下真实判级要 ~40s+。同步路径不传它（socket 维持 20s、30s 契约零变化，老客户端行为不动）；坏 JSON 回喂重试由 120s 墙钟兜底收尾 |
